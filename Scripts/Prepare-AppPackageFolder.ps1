...
.SYNOPSIS
    ...
    - Copy app-specific Invoke-AppDeployToolkit.ps1 to package folder
...
.NOTES
    ...
    1.0.2 - (2024-03-04) Added ProductCode string replacement for Invoke-AppDeployToolkit.ps1
#>
[CmdletBinding(SupportsShouldProcess = $true)]
param (
    [ValidateNotNullOrEmpty()]
    [string]$SCRIPTSIGNIN = $env:SCRIPTSIGNIN
)
Process {
    ...
    if (Test-Path -Path $AppsPrepareListFilePath) {
        ...
        foreach ($App in $AppsPrepareList) {
            ...
            try {
                ...
                if (-not(Test-Path -Path $AppPublishFolderPath)) {
                    ...
                    New-Item -Path $AppPublishFolderPath -ItemType "Directory" -Force -Confirm:$false -ErrorAction Stop | Out-Null
                }
                ...
                Copy-Item -Path "$($FrameworkPath)\*" -Destination $AppPublishFolderPath -Recurse -Force -Confirm:$false -ErrorAction Stop
                ...
                if (-not(Test-Path -Path $AppsPublishSourceFilesPath)) {
                    New-Item -Path $AppsPublishSourceFilesPath -ItemType "Directory" -Force -Confirm:$false -ErrorAction Stop | Out-Null
                }

                # Copy additional config files 
                $AppConfigFilesFolderPath = Join-Path -Path $SourceDirectory -ChildPath "Apps\$($App.AppFolderName)\ConfigFiles"
                if (Test-Path -Path $AppConfigFilesFolderPath) {Copy-Item -Path "$($AppConfigFilesFolderPath)\*" -Destination "$($AppsPublishSourceFilesPath)\" -Recurse -Force -Confirm:$false -ErrorAction Stop}
                ...
                Copy-Item -Path "$($App.AppSetupFolderPath)\*" -Destination "$($AppPublishFolderPath)\Source\Files\" -Recurse -Force -Confirm:$false -ErrorAction Stop
                ...
                $AppFileNames = @("App.json", "Invoke-AppDeployToolkit.ps1", "Icon.png")
                foreach ($AppFileName in $AppFileNames) {
                    ...
                    switch ($AppFileName) {
                        "Invoke-AppDeployToolkit.ps1" {
                            ...
                            Out-File -InputObject $AppFileContent -FilePath $AppDestinationFilePath -Encoding "utf8" -Force -Confirm:$false -ErrorAction Stop
                        }
                        "App.json" {
                            ...
                            foreach ($DetectionRuleItem in $AppFileContent.DetectionRule) {
                                switch ($DetectionRuleItem.Type) {
                                    ...
                                    "Script" {
                                        ...
                                        Copy-Item -Path $AppDetectionScriptFile -Destination $AppDetectionScriptFileDestinationPath -Force -Confirm:$false -ErrorAction Stop
                                        ...
                                        $AppDetectionScriptFileContent = Get-Content -Path $AppDetectionScriptFileDestinationPath -ErrorAction Stop
                                        ...
                                        Out-File -InputObject $AppDetectionScriptFileContent -FilePath $AppDetectionScriptFileDestinationPath -Encoding "utf8" -Force -Confirm:$false -ErrorAction Stop
                                    }
                                }
                            }
                            ...
                            Out-File -InputObject ($AppFileContent | ConvertTo-Json) -FilePath $AppFileDestinationPath -Encoding "utf8" -Force -Confirm:$false -ErrorAction Stop
                        }
                        "Icon.png" {
                            ...
                            if (-not([string]::IsNullOrEmpty($App.IconURL))) {
                                ...
                                Invoke-WebRequest -Uri $App.IconURL -OutFile $IconFilePath -UseBasicParsing -ErrorAction Stop
                            }
                            else {
                                ...
                                Copy-Item -Path $AppFilePath -Destination $AppFileDestinationPath -Force -Confirm:$false -ErrorAction Stop
                            }
                    ...
                    #Script Signin logic
                    $certificateBasePath = $env:LOCALAPPDATA
                    #$certificatePath = Join-Path -Path (Join-Path -Path $env:BUILD_SOURCESDIRECTORY -ChildPath "CodeSigningCertificates") -ChildPath "WPS_APPS_Packaging.pfx"
                    $certificatePath = Join-Path -Path (Join-Path -Path $certificateBasePath -ChildPath "CodeSigningCertificates") -ChildPath "WPS_APPS_Packaging.pfx"
                    $certCollection = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2Collection
                    $certCollection.Import($certificatePath, $SCRIPTSIGNIN, [System.Security.Cryptography.X509Certificates.X509KeyStorageFlags]::DefaultKeySet)
                    $cert = $certCollection[0]
                    $FilesToSign = Get-ChildItem -Path "$AppPublishFolderPath\Source\*" -File -Recurse | Where-Object {$_.Extension -in '.ps1', '.psd1', '.psm1'}
                    ForEach ($File in $FilesToSign) {
                        Set-AuthenticodeSignature -FilePath $File.FullName -Certificate $cert -TimeStampServer "http://timestamp.digicert.com" -HashAlgorithm "SHA256" -ErrorAction Stop
                    }
                ...
                if (-not(Test-Path -Path $AppRootFolderPackagePath)) {
                    ...
                    New-Item -Path $AppRootFolderPackagePath -ItemType "Directory" -Force -Confirm:$false -ErrorAction Stop | Out-Null
                }

            } 
            catch {
                # Log the error and mark processing as failed
                Write-Warning -Message "Processing of application '$($App.IntuneAppName)' failed: $($_.Exception.Message)"
                $AppProcessingSucceeded = $false
                
            } 
            finally {
                if ($AppProcessingSucceeded) {
                    ...
                    Write-Output -InputObject "[APPLICATION: $($App.IntuneAppName)] - Failed, skipping add to publish list"
                    # Optional: cleanup partially created folder(s) if desired
                    # Remove-Item -Path $AppPublishFolderPath -Recurse -Force -ErrorAction SilentlyContinue
                }
            }
        }
        ...
        if ($AppsPublishList.Count -ge 1) {
            ...
            $FailedApps = $AppsPrepareList | Where-Object { $_.IntuneAppName -notin $AppsPublishList.IntuneAppName }
            if($FailedApps.Count -ne 0) {Update-CatlogueStatus -AccessToken $token -Apps $FailedApps -Reason "IAF - PSADT package creation failed"}
        }
        ...
        if ($AppsPublishList.Count -eq 0) {
            ...
            $BinaryLocation = Join-Path -Path $env:BUILD_BINARIESDIRECTORY -ChildPath "AppsPrepareList.json"
            Update-CatlogueStatus -AccessToken $token -Apps "All" -InputFilePath $BinaryLocation  -Reason "IAF - PSADT package creation failed"
