...
param (
    ...
    [ValidateNotNullOrEmpty()]
    [string]$WorkspaceID = $env:IntuneWorkspaceID,

    [ValidateNotNullOrEmpty()]
    [string]$SharedKey = $env:LA_IAF_PrimaryKey
)
Process {
    ...
    if (Test-Path -Path $AppsPublishListFilePath) {
        ...
        foreach ($App in $AppsPublishList) {
            ...
            if (-not([string]::IsNullOrEmpty($AppData.Information.Notes))) {
                if ($AppData.Information.Notes -is [array]) { $AppData.Information.Notes = $AppData.Information.Notes -join "`n" }
            ...
            try {
                ...
                try {
                    ...
                    # Skip app if App Object ID not generated
                    Write-Host "Upload:  $($App.IntuneAppName) [$($Win32App.id)]"
                    if ([string]::IsNullOrWhiteSpace($Win32App.id)) {
                        Write-Output "Failed to upload the App $($App.IntuneAppName)"
                        Update-CatlogueStatus -AccessToken $token -Apps $App -Reason "IAF - Package Publish to Intune failed"
                        continue
                    }
                    ...
                    Update-CatlogueStatus -AccessToken $token -Apps $App -Reason "IAF - Package Publish to Intune failed" #Update Catlogue
                }
            }
            catch [System.Exception] {
            ...
            # Add sleep here to prevent throttling or session issues
        ...
        if ($AppsAssignList.Count -eq 0) {
            ...
            Update-CatlogueStatus -AccessToken $token -Apps "All" -InputFilePath $BinaryLocation  -Reason "IAF - Package Publish to Intune failed"
