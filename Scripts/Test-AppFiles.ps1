...
#>
# Import the module (relative path)
Import-Module "$PSScriptRoot/Add-AppsFailureLog.psm1"
...
$AppFileNames = @("App.json", "Invoke-AppDeployToolkit.ps1")
...
try {
    ...
    foreach ($App in $AppList.Apps) {
        ...
        if (-not(Test-Path -Path $AppPackageFolderPath)) {
            ...
            $Log = "Could not find expected application directory path: $($AppPackageFolderPath)"
            $AppAllowed = $false
        }
        else {
            ...
            foreach ($AppFileName in $AppFileNames) {
                ...
                if (-not(Test-Path -Path $AppFileNamePath)) {
                    ...
                    $Log = "Could not find required file: $($AppFileName) for $($App.IntuneAppName) in Apps on GitHub [intune-appfactory-Packages/IAF-App-Onboarding-ADT]"
                    $AppAllowed = $false
                }
                else {
                    ...
                    switch ($AppFileName) {
                        "App.json" {    
                            ...
                            if ($AppFileContent.DetectionRule.Count -eq 0) {
                                ...
                                $Log = "Could not find any detection rule defined, ensure App.json contains atleast one detection rule element"
                                $AppAllowed = $false
                            }
                            ...
                            if ($AppFileContent.PackageInformation.IconURL -ne $null) {
                                if (-not([string]::IsNullOrEmpty($AppFileContent.PackageInformation.IconURL))) {
                                    try {
                                        ...
                                        $Log = "Could not access icon URL: $($IconURL)"
                                        $AppAllowed = $false
                                    }
                                }
                                else {
                                    ...
                                    if (-not(Test-Path -Path $IconFilePath)) {
                                        ...
                                        $Log = "Could not detect local Icon.png file in app folder"
                                        $AppAllowed = $false
                                    }
                                    else {
                            ...
                            if ($AppFileContent.DetectionRule.Count -eq 1) {
                                if ($AppFileContent.DetectionRule.Type -like "Script") {
                                    ...
                                    if (-not(Test-Path -Path $DetectionScriptFilePath)) {
                                        ...
                                        $Log = "Could not detect given detection script file in app folder: $($AppFileContent.DetectionRule.ScriptFile)"
                                        $AppAllowed = $false
                                    }
                                }
                            }
                            ...
                            if ($AppFileContent.DetectionRule.Count -ge 2) {
                                if ("Script" -in $AppFileContent.DetectionRule.Type) {
                                    ...
                                    $Log = "Multiple detection rule types are defined in App.json, where at least one of them are of type 'Script', which is not a supported configuration in Intune"
                                    $AppAllowed = $false
                                }
                            }
                        }
                ...
                if (($AppAllowed -eq $false) -and ($Log -ne '')){Add-AppsFailureLog -AppName $App.IntuneAppName  -Log $Log} #Added Logging for Apps based on errors
    ...
    Write-Output "PS_ERROR_DESC= $($MyInvocation.MyCommand): Failed to access '$($AppListPath)' with error message: $($_.Exception.Message)"
    exit 1
}
