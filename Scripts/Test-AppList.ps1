...
Process {
    ...
    function Get-EvergreenAppItem {
        ...
        if ($PSBoundParameters["FilterOptions"]) {
            ...
            #Added Stream for CitrixWorkspaceApp and wireshark
            if ($FilterOptions.Stream) {
                $FilterList.Add("`$PSItem.Stream -eq ""$($FilterOptions.Stream)""") | Out-Null
            }
    ...
    function Get-WindowsPackageManagerItem {
        ...
        process {
            ...
            # Write-Host "AppId1 " + $AppId
            [string[]]$WinGetArguments = @("search", "$($AppId)")
            # Write-Host "Application details1" + $WinGetArguments
            ...
            if ($AppResult -eq $true) {
                ...
                # Write-Host "AppId2" + $AppId
                [string[]]$WinGetArguments = @("show", "$($AppId)")
                # Write-Host "Application details1" + $WinGetArguments
                # Write-Host "Application details2" + $WinGetArguments
                [string[]]$WinGetStream = & "winget" $WinGetArguments | Out-String -Stream
                        ...
                        try {
                            ...
                            switch ($App.IntuneAppNamingConvention) {
                                "PublisherAppNameAppVersion" {
                                    ...
                                    #$AppDisplayName = -join@($App.AppPublisher, " ", $App.IntuneAppName) #[Artur]
                                }
                                "PublisherAppName" {
                                ...
                                "AppNameAppVersion" {
                                    ...
                                    #$AppDisplayName = $App.IntuneAppName #[Artur]
                                }
                                "AppName" {
