...
function Prepare_EmailBody_TableContent {
    ...
    try {
        ...
        if ($pickemailtemplate -eq "Failed") {
            ...
            $templatePath = "$($env:WORKSPACE)\Templates\EmailTemplates\CombinedEmail_Failure.html"
            Write-Host "Using Failure template"
        } elseif ($pickemailtemplate -eq "IAT") {
            # IAT_Testing is set - use IAT template
            $templatePath = "$($env:WORKSPACE)\Templates\EmailTemplates\CombinedEmail_IATTest.html"
            Write-Host "Using IAT template"
        } elseif ($pickemailtemplate -eq "AO") {
            # IAT_Testing is null/empty - use AO template
            $templatePath = "$($env:WORKSPACE)\Templates\EmailTemplates\CombinedEmail_AOTest.html"
...
function Get-IATTestingStatus {
    ...
    try {
        ...
        $AccessToken = Get-CatalogueAccessToken -username $env:APP_CATALOGUE_USERNAME -password $env:APP_CATALOGUE_PASSWORD
