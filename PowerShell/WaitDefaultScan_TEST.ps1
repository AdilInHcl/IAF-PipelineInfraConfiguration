...
try{
    ...
    $UnderScanningDevices = $deviceScanInfoList
    ...
    for ($retryCount = 0; $retryCount -le $maxRetries; $retryCount++) {
        ...
        foreach ($vm in $UnderScanningDevices){
            ...
            if (($vm.UIStatus -eq "Updated") -and $lastscantimestamp){
                Write-Host "Default Scan: Qualys Completed [$lastscantimestamp] for $vmName ( $assetID )"

            
                $scantime = Get-Date  #Fetch the TImestamp scan was triggered                
                $vm.Status = "$ScanTriggerStatus : Qualys Default scan Completed [$scantime] for $vmName ( $assetID )"
                $vm.ScanTriggered = "YES"
                
            }
        }

        #Skip the Iteration  of the Completed Scan
        $UnderScanningDevices = $UnderScanningDevices | Where-Object { $_.ScanTriggered -ne "YES" }
